...
- [Contributing to Drive By Fix](#contributing-to-drive-by-fix)
...
```bat
cd %userprofile%\Documents\Projects\pawn-drive-by-fix\
```

On Linux:

```bash
cd ~/pawn-drive-by-fix/
```

Clone via SSH:  

```bash
git clone git@github.com:San-Andreas-Online/pawn-drive-by-fix.git
```

Clone via HTTPS

```bash
git clone https://github.com/San-Andreas-Online/pawn-drive-by-fix.git
```
